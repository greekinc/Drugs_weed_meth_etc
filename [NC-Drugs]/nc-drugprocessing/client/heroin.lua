...
local function ProcessHeroin()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nc-drugprocessing:processPoppyResin')
...
RegisterNetEvent('nc-drugprocessing:ProcessPoppy', function()
...
RegisterNetEvent("nc-drugprocessing:processHeroin",function()
	QBCore.Functions.TriggerCallback('nc-drugprocessing:validate_items', function(result)
		if result.ret then
			ProcessHeroin()
		else
...
RegisterNetEvent("nc-drugprocessing:pickHeroin", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		...
		QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
			...
			TriggerServerEvent('nc-drugprocessing:pickedUpPoppy')
			isPickingUp = false
