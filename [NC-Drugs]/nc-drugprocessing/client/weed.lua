...
local function RollJoint()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.rolling_joint"), 15000, false, true, {
		...
		TriggerServerEvent('nc-drugprocessing:rollJoint')
...
local function ProcessWeed()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nc-drugprocessing:processCannabis')
		local timeLeft = Config.Delays.WeedProcessing / 1000
		while timeLeft > 0 do
			Wait(1000)
			timeLeft -= 1
...
RegisterNetEvent("nc-drugprocessing:processWeed",function()
...
RegisterNetEvent("nc-drugprocessing:pickWeed", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
				...
				spawnedWeeds -= 1
				TriggerServerEvent('nc-drugprocessing:pickedUpCannabis')
				isPickingUp = false
			end, function()
				ClearPedTasks(PlayerPedId())
				isPickingUp = false
...
RegisterNetEvent('nc-drugprocessing:client:rollJoint', function()
    QBCore.Functions.TriggerCallback('nc-drugprocessing:validate_items', function(result)
		if result.ret then
			RollJoint()
		else
