...
local function ProcessCoke()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nc-drugprocessing:processCocaLeaf')
...
local function CutCokePowder()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nc-drugprocessing:processCocaPowder')
...
local function ProcessBricks()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('nc-drugprocessing:processCocaBrick')
...
RegisterNetEvent('nc-drugprocessing:ProcessCocaFarm', function()
...
RegisterNetEvent('nc-drugprocessing:ProcessCocaPowder', function()
	local coords = GetEntityCoords(PlayerPedId())
	local amount = 10
	local amount2 = 5
...
RegisterNetEvent('nc-drugprocessing:ProcessBricks', function()
...
RegisterNetEvent('nc-drugprocessing:EnterCWarehouse', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('nc-drugprocessing:ExitCWarehouse', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
...
RegisterNetEvent('nc-drugprocessing:pickCocaLeaves', function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
				...
				TriggerServerEvent('nc-drugprocessing:pickedUpCocaLeaf')
				isPickingUp = false
			end, function()
				ClearPedTasks(playerPed)
				isPickingUp = false
