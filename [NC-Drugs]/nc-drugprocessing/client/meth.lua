...
local function ProcessChemicals()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('nc-drugprocessing:processChemicals')
...
local function ProcessTempUp()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_up"), 15000, false, true, {
		...
		TriggerServerEvent('nc-drugprocessing:processTempUp')
...
local function ProcessTempDown()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.temp_down"), 15000, false, true, {
		...
		TriggerServerEvent('nc-drugprocessing:processTempDown')
...
local function ProcessProduct()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.packing"), 15000, false, true, {
		...
		TriggerServerEvent('nc-drugprocessing:processMeth')
...
RegisterNetEvent('nc-drugprocessing:ProcessChemicals', function()
...
RegisterNetEvent('nc-drugprocessing:ChangeTemp', function()
	if not isTempChangeU then
		QBCore.Functions.TriggerCallback('nc-drugprocessing:validate_items', function(result)
			if result.ret then
				exports['nc-ui']:Thermite(function(success)
					if success then
						QBCore.Functions.Notify(Lang:t("success.temp_up"), 'success')
						ProcessTempUp()
					else
						TriggerServerEvent('nc-drugprocessing:cancelProcessing')
						TriggerServerEvent('nc-drugprocessing:processFailUp')
					end
				end, 10, 5, 3)
			else
				QBCore.Functions.Notify(Lang:t("error.no_item", {item = result.item}))
...
RegisterNetEvent('nc-drugprocessing:ChangeTemp2', function()
			...
			if result.ret then
				exports['nc-ui']:Thermite(function(success)
					if success then
						...
						TriggerServerEvent('nc-drugprocessing:cancelProcessing')
						TriggerServerEvent('nc-drugprocessing:processFailDown')
					end
				end, 10, 5, 3)
			else
				QBCore.Functions.Notify(Lang:t("error.no_item", {item = result.item}))
...
RegisterNetEvent('nc-drugprocessing:ProcessProduct', function()
	local coords = GetEntityCoords(PlayerPedId(source))
...
RegisterNetEvent('nc-drugprocessing:EnterLab', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('nc-drugprocessing:ExitLab', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
